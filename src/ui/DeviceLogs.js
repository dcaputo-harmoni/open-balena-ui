...
const handleSubmit = values => {
    ...
    return fetch(`${API_HOST}/device/v2/${props.record.uuid}/logs`, {
        ...
        return reader.read().then(function processText({ done, value }) {
            if (done) {
                const logs = JSON.parse(stringFromUTF8Array(result));
                let containerLogs = values.container === 0
                ? logs.filter(x => !x.hasOwnProperty("serviceId")).map(x => x.message).join('<br/>')
                : logs.filter(x => x.serviceId === values.container).map(x => x.message).join('<br/>');
                containerLogs = "<html style='font-family: consolas; color: #ffffff'>" + containerLogs + "</html>";
                setContent(containerLogs);
                return;
            }
            result.push(...value);
    ...
    if (!loaded) {
        ...
        dataProvider.getList('image install', {
            ...
            Promise.all(installs.data.map(async install => {
                ...
                console.dir({id: image.service.data[0].id, name: containerName});
