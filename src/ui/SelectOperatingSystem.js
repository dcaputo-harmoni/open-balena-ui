    ...
    React.useEffect(() => {
        if (loaded === null) {
            ...
            dataProvider.getList('application', {
                ...
                Promise.all(operatingSystems.data.map(operatingSystem => (
                    dataProvider.getList('release', {
                        pagination: { page: 1 , perPage: 1000 },
                        sort: { field: 'id', order: 'ASC' },
                        filter: { "belongs to-application": operatingSystem.id }
                    }).then(operatingSystemReleases => {
                        return Promise.all(operatingSystemReleases.data.map(operatingSystemRelease => 
                            dataProvider.getList('release tag', {
                                pagination: { page: 1 , perPage: 1000 },
                                sort: { field: 'id', order: 'ASC' },
                                filter: { "release": operatingSystemRelease.id, "tag key": "version" }
                            }).then(operatingSystemReleaseTag => {
                                operatingSystemOpts.push({ name: operatingSystemReleaseTag.data[0].value, id: operatingSystemRelease.id})
                            })
                        ));
                    })
                ))).then(() => {
